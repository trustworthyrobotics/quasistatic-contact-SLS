...
template <>
struct Traits<multibody::quasidynamic::QuasidynamicMultibodyPlant>
...
struct RigidBody {
  explicit RigidBody(const SpatialInertia<double>& M_BBo_B_in)
      : M_BBo_B(M_BBo_B_in) {}
...
struct Joint {
  enum class Type { Revolute, Prismatic, Cartesian, RpyFloating };

  template <typename Derived>
  Joint(Type type_in, BodyIndex body_P_in,
        const math::RigidTransform<double>& X_PJp_in, BodyIndex body_C_in,
        const math::RigidTransform<double>& X_CJc_in,
        const std::optional<Vector3<double>>& axis_in,
  ...
  Type type{};
  BodyIndex body_P{};
  math::RigidTransform<double> X_PJp;
  BodyIndex body_C{};
  math::RigidTransform<double> X_CJc;
  std::optional<Vector3<double>> axis;
  std::optional<Eigen::VectorXd> actuation_stiffness;
};

struct CollisionGeometry {
  CollisionGeometry(BodyIndex body_in,
                    const math::RigidTransform<double>& X_BG_in,
                    const geometry::Shape& shape_in,
  ...
  copyable_unique_ptr<geometry::Shape> shape;
...
template <typename T>
class QuasidynamicMultibodyPlant<T>::Impl {
 public:
  ...
  Impl() {
    ...
    bodies_.emplace_back(
        std::make_unique<RigidBody>(SpatialInertia<double>::Zero()));
  }

  BodyIndex AddRigidBody(const SpatialInertia<double>& M_BBo_B) {
    DRAKE_THROW_UNLESS(!finalized_);
    bodies_.emplace_back(std::make_unique<RigidBody>(M_BBo_B));
  ...
  JointIndex AddRevoluteJoint(BodyIndex body_P,
                              const math::RigidTransform<double>& X_PJp,
                              BodyIndex body_C,
                              const math::RigidTransform<double>& X_CJc,
  ...
  JointIndex AddRpyFloatingJoint(
      BodyIndex body_P, const math::RigidTransform<double>& X_PJp,
      BodyIndex body_C, const math::RigidTransform<double>& X_CJc,
      const std::optional<Vector6<double>>& actuation_stiffness) {
    DRAKE_THROW_UNLESS(!finalized_);
    DRAKE_THROW_UNLESS(!actuation_stiffness ||
                       (actuation_stiffness->array() > 0).all());
    return AddJoint(Joint(Joint::Type::RpyFloating, body_P, X_PJp, body_C,
                          X_CJc, {}, actuation_stiffness));
  }

  GeometryId RegisterCollisionGeometry(BodyIndex body,
  ...
  void Finalize() {
    ...
    for (BodyIndex i{1}; i < ssize(bodies_); ++i) {
      if (!bodies_[i]->parent) {
        AddRpyFloatingJoint(world_body(), math::RigidTransform<double>(), i,
                            math::RigidTransform<double>(), std::nullopt);
      }
    }
    ...
    const std::function<void(BodyIndex, bool)> recursive_check =
        [this, &visited, &recursive_check](BodyIndex body_index,
          ...
          if (!actuated && (body.M_BBo_B.IsZero() || body.M_BBo_B.IsNaN())) {
            throw std::runtime_error(fmt::format(
                "Rigid body {} is unactuated but has zero spatial inertia",
                body_index));
          }
    ...
    /* Make the contact pairs and the
  ...
  const math::RigidTransform<T>& EvalPoseInWorld(const Scratch<T>& scratch,
                                                 BodyIndex body_index) const {
    DRAKE_THROW_UNLESS(finalized_);
    DRAKE_THROW_UNLESS(body_index < bodies_.size());
    return multibody_tree()
        .get_body(body_index)
        .EvalPoseInWorld(scratch.context());
  }

  Eigen::SparseMatrix<T> CalcQDotToVelocityMap(
      const Scratch<T>& scratch) const {
    return multibody_tree().MakeQDotToVelocityMap(scratch.context());
  }
  ...
  void CalcContact(const Scratch<T>& scratch, EigenPtr<MatrixX<T>> J,
    ...
    DRAKE_THROW_UNLESS(J->rows() == 3 * num_contacts() &&
                       J->cols() == num_dofs());
    ...
    for (int i = 0; i < num_contacts(); ++i) {
      ...
      const geometry::Shape& shape_A = *geometry_A.shape;
      const geometry::Shape& shape_B = *geometry_B.shape;
      const math::RigidTransform<T> X_WA =
          EvalPoseInWorld(scratch, geometry_A.body) * geometry_A.X_BG.cast<T>();
      const math::RigidTransform<T> X_WB =
          EvalPoseInWorld(scratch, geometry_B.body) * geometry_B.X_BG.cast<T>();
      const geometry::SignedDistancePair<T> res =
          CalcProximity(shape_A, X_WA, shape_B, X_WB, smoothing);
      ...
      multibody_tree().CalcJacobianTranslationalVelocity(
          ...
          res.p_ACa, frame_W, frame_W, &Js_v_W);
      const Matrix3<T> R_WCa = math::RotationMatrix<T>::MakeFromOneUnitVector(
                                   res.nhat_BA_W, /* x-axis */ 0)
                                   .matrix();
      J->template middleRows<3>(3 * i).leftCols(num_actuated_dofs()) =
          R_WCa.transpose() * Js_v_W.leftCols(num_actuated_dofs());

      const auto& frame_B =
          multibody_tree().get_body(geometry_B.body).body_frame();
      multibody_tree().CalcJacobianTranslationalVelocity(
          scratch.context(), JacobianWrtVariable::kV, frame_B, frame_B,
          res.p_BCb, frame_W, frame_W, &Js_v_W);
      /* The contact force has equal magnitude and opposite direction, so simply
       switch the sign of each axis. */
      const Matrix3<T> R_WCb = -R_WCa;
      J->template middleRows<3>(3 * i).rightCols(num_unactuated_dofs()) =
          R_WCb.transpose() * Js_v_W.rightCols(num_unactuated_dofs());
    }
  }
  ...
  void CalcCollision(const Scratch<T>& scratch, EigenPtr<MatrixX<T>> J,
    ...
    for (int i = 0; i < num_collisions(); ++i) {
      ...
      const math::RigidTransform<T> X_WB =
          EvalPoseInWorld(scratch, geometry_B.body) * geometry_B.X_BG.cast<T>();
      const geometry::SignedDistancePair<T> res =
          CalcProximity(shape_A, X_WA, shape_B, X_WB);

      (*phi)[i] = res.distance;
      const Matrix3<T> R_WC = math::RotationMatrix<T>::MakeFromOneUnitVector(
                                  res.nhat_BA_W, /* x-axis */ 0)
                                  .matrix();
      ...
      multibody_tree().CalcJacobianTranslationalVelocity(
          scratch.context(), JacobianWrtVariable::kV, frame_A, frame_A,
          res.p_ACa, frame_W, frame_W, &Js_v_W);
      J->row(i) += R_WC.transpose().row(0) * Js_v_W;
  ...
  systems::EventStatus DrawMeshcat(const Scratch<T>& scratch,
    ...
    for (BodyIndex body_index{0}; body_index < ssize(bodies_); ++body_index) {
      ...
      const math::RigidTransform<T> pose = EvalPoseInWorld(scratch, body_index);
      const math::RigidTransform<double> X_WB(
          math::RotationMatrix(ExtractDoubleOrThrow(pose.rotation().matrix())),
          ExtractDoubleOrThrow(pose.translation()));

      for (int i = 0; i < ssize(body.visual_geometries); ++i) {
        const GeometryId geometry_id = body.visual_geometries[i];
        const VisualGeometry& geometry = visual_geometries_.at(geometry_id);
        const std::string name =
            geometry.name
                ? fmt::format("/{}/{}", name_prefix, geometry.name.value())
                : fmt::format("/{}/body{}/geometry{}", name_prefix, body_index,
                              i);
        const geometry::Shape& shape = *geometry.shape;
        const geometry::Rgba& rgba = geometry.rgba;
        const math::RigidTransform<double> X_WG = X_WB * geometry.X_BG;

        meshcat.SetObject(name, shape, rgba);
  ...
  template <typename U>
  void CopyFrom(const typename QuasidynamicMultibodyPlant<U>::Impl& other) {
 ...
 private:
  /* Allows QuasidynamicMultibodyPlant<U>::Impl to access private members of
   QuasidynamicMultibodyPlant<T>::Impl for copying. */
  template <typename U>
  ...
  std::unique_ptr<internal::MultibodyTreeSystem<T>> MakeMultibodyTreeSystem()
    ...
    for (BodyIndex body_index{1}; body_index < ssize(bodies_); ++body_index) {
      const RigidBody& body = *bodies_[body_index];
      const auto& res = tree->AddRigidBody(
          fmt::format("RigidBody{}", body_index), body.M_BBo_B);
    ...
    for (JointIndex joint_index : joint_indexes) {
      ...
      const math::RigidTransform<double>& X_PJp = joint.X_PJp;
      const auto& child = tree->get_body(joint.body_C);
      const math::RigidTransform<double>& X_CJc = joint.X_CJc;
      const Vector3<double> axis =
          joint.axis ? joint.axis.value() : Vector3<double>::Zero();

      int dof = -1;
      switch (joint.type) {
        case Joint::Type::Revolute: {
          dof = 1;
          tree->template AddJoint<RevoluteJoint>(name, parent, X_PJp, child,
                                                 X_CJc, axis);
          break;
        }
        case Joint::Type::Prismatic: {
          dof = 1;
          tree->template AddJoint<PrismaticJoint>(name, parent, X_PJp, child,
                                                  X_CJc, axis);
          break;
        }
        case Joint::Type::Cartesian: {
          dof = 3;
          const auto& intermediate1 = tree->AddRigidBody(
              fmt::format("IntermediateRigidBody{}_1", joint_index),
              SpatialInertia<double>::Zero());
          const auto& intermediate2 = tree->AddRigidBody(
              fmt::format("IntermediateRigidBody{}_2", joint_index),
              SpatialInertia<double>::Zero());
          tree->template AddJoint<PrismaticJoint>(
              name, parent, X_PJp, intermediate1, {}, Vector3<double>::UnitX());
          tree->template AddJoint<PrismaticJoint>(name + "_1", intermediate1,
                                                  {}, intermediate2, {},
                                                  Vector3<double>::UnitY());
          tree->template AddJoint<PrismaticJoint>(name + "_2", intermediate2,
                                                  {}, child, X_CJc,
                                                  Vector3<double>::UnitZ());
          break;
        }
        case Joint::Type::RpyFloating: {
          dof = 6;
          tree->template AddJoint<RpyFloatingJoint>(name, parent, X_PJp, child,
                                                    X_CJc);
          break;
        }
        default:
    ...
    DRAKE_DEMAND(tree->num_velocities() == tree->num_positions());
  ...
  Vector3<double> gravity_{0, 0, -9.80665};
...
template <typename T>
void QuasidynamicMultibodyPlant<T>::ImplDeleter::operator()(Impl* impl) {
  delete impl;
}

template <typename T>
QuasidynamicMultibodyPlant<T>::QuasidynamicMultibodyPlant(
    double dt, double dynamics_smoothing, double geometry_smoothing)
    : QuasidynamicDifferentiableContactModel<T>(
          systems::SystemTypeTag<QuasidynamicMultibodyPlant>{},
          dynamics_smoothing),
      dt_(dt),
...
template <typename T>
BodyIndex QuasidynamicMultibodyPlant<T>::AddRigidBody(
    const SpatialInertia<double>& M_BBo_B) {
  DRAKE_DEMAND(impl_ != nullptr);
  return impl_->AddRigidBody(M_BBo_B);
}

template <typename T>
BodyIndex QuasidynamicMultibodyPlant<T>::world_body() const {
  DRAKE_DEMAND(impl_ != nullptr);
  return impl_->world_body();
}

template <typename T>
JointIndex QuasidynamicMultibodyPlant<T>::AddRevoluteJoint(
    BodyIndex body_P, const math::RigidTransform<double>& X_PJp,
    BodyIndex body_C, const math::RigidTransform<double>& X_CJc,
    const Vector3<double>& axis, std::optional<double> actuation_stiffness) {
  DRAKE_DEMAND(impl_ != nullptr);
  return impl_->AddRevoluteJoint(body_P, X_PJp, body_C, X_CJc, axis,
                                 actuation_stiffness);
}

template <typename T>
JointIndex QuasidynamicMultibodyPlant<T>::AddPrismaticJoint(
    BodyIndex body_P, const math::RigidTransform<double>& X_PJp,
    BodyIndex body_C, const math::RigidTransform<double>& X_CJc,
    const Vector3<double>& axis, std::optional<double> actuation_stiffness) {
  DRAKE_DEMAND(impl_ != nullptr);
  return impl_->AddPrismaticJoint(body_P, X_PJp, body_C, X_CJc, axis,
                                  actuation_stiffness);
}

template <typename T>
JointIndex QuasidynamicMultibodyPlant<T>::AddCartesianJoint(
    BodyIndex body_P, const math::RigidTransform<double>& X_PJp,
    BodyIndex body_C, const math::RigidTransform<double>& X_CJc,
    const std::optional<Vector3<double>>& actuation_stiffness) {
  DRAKE_DEMAND(impl_ != nullptr);
...
template <typename T>
JointIndex QuasidynamicMultibodyPlant<T>::AddRpyFloatingJoint(
    BodyIndex body_P, const math::RigidTransform<double>& X_PJp,
    BodyIndex body_C, const math::RigidTransform<double>& X_CJc,
    const std::optional<Vector6<double>>& actuation_stiffness) {
  DRAKE_DEMAND(impl_ != nullptr);
  return impl_->AddRpyFloatingJoint(body_P, X_PJp, body_C, X_CJc,
                                    actuation_stiffness);
}

template <typename T>
GeometryId QuasidynamicMultibodyPlant<T>::RegisterCollisionGeometry(
...
template <typename T>
GeometryId QuasidynamicMultibodyPlant<T>::RegisterVisualGeometry(
    BodyIndex body, const math::RigidTransform<double>& X_BG,
...
template <typename T>
void QuasidynamicMultibodyPlant<T>::SetGravityVector(
    const Vector3<double>& g_W) {
  DRAKE_DEMAND(impl_ != nullptr);
  impl_->SetGravityVector(g_W);
}

template <typename T>
void QuasidynamicMultibodyPlant<T>::Finalize() {
  DRAKE_DEMAND(impl_ != nullptr);
  ...
  qdot_to_vel_map_cache_index_ =
      this->DeclareCacheEntry(
              "qdot_to_vel_map",
              QuasidynamicMultibodyPlantSparseMatrix<T>(num_dofs, num_dofs),
              &QuasidynamicMultibodyPlant<T>::CalcQDotToVelocityMap,
  ...
  M_unactuated_cache_index_ =
      this->DeclareCacheEntry(
              ...
              &QuasidynamicMultibodyPlant<T>::CalcUnactuatedMassMatrix,
              {this->all_state_ticket()})
          .cache_index();
  ...
  P_quasidynamic_cache_index_ =
      this->DeclareCacheEntry(
              ...
              &QuasidynamicMultibodyPlant<T>::CalcQuasidynamicPMatrix,
              {this->all_state_ticket()})
          .cache_index();
  ...
  b_quasidynamic_cache_index_ =
      this->DeclareCacheEntry(
              ...
              &QuasidynamicMultibodyPlant<T>::CalcQuasidynamicBVector,
  ...
  contact_cache_index_ =
      this->DeclareCacheEntry(
              "contact",
              std::make_tuple(
                  MatrixX<T>(MatrixX<T>::Zero(3 * num_contacts, num_dofs)),
                  VectorX<T>(VectorX<T>::Zero(num_contacts))),
              &QuasidynamicMultibodyPlant<T>::CalcContact,
              {this->all_state_ticket(),
  ...
  collision_cache_index_ =
      this->DeclareCacheEntry(
              ...
              &QuasidynamicMultibodyPlant<T>::CalcCollision,
              {this->all_state_ticket()})
          .cache_index();
  ...
  this->DeclarePerStepPublishEvent(&QuasidynamicMultibodyPlant<T>::DrawMeshcat);
  this->DeclareForcedPublishEvent(&QuasidynamicMultibodyPlant<T>::DrawMeshcat);
}

template <typename T>
double QuasidynamicMultibodyPlant<T>::get_geometry_smoothing(
    const systems::Context<T>& context) const {
...
template <typename T>
void QuasidynamicMultibodyPlant<T>::set_geometry_smoothing(
...
template <typename T>
static const Scratch<T>& EvalScratch(
    const QuasidynamicMultibodyPlant<T>& system,
...
template <typename T>
void QuasidynamicMultibodyPlant<T>::CalcQDotToVelocityMap(
    const systems::Context<T>& context,
    QuasidynamicMultibodyPlantSparseMatrix<T>* N) const {
  DRAKE_DEMAND(impl_ != nullptr);
  const Scratch<T>& scratch = EvalScratch(*this, context, scratch_cache_index_);
  N->matrix = impl_->CalcQDotToVelocityMap(scratch);
  DRAKE_DEMAND(N->matrix.cols() == N->matrix.rows());
}

template <typename T>
const Eigen::SparseMatrix<T>&
QuasidynamicMultibodyPlant<T>::EvalQDotToVelocityMap(
    const systems::Context<T>& context) const {
  return this->get_cache_entry(qdot_to_vel_map_cache_index_)
      .template Eval<QuasidynamicMultibodyPlantSparseMatrix<T>>(context)
      .matrix;
}

template <typename T>
void QuasidynamicMultibodyPlant<T>::CalcUnactuatedMassMatrix(
...
template <typename T>
const MatrixX<T>& QuasidynamicMultibodyPlant<T>::EvalUnactuatedMassMatrix(
    const systems::Context<T>& context) const {
...
template <typename T>
void QuasidynamicMultibodyPlant<T>::CalcQuasidynamicPMatrix(
    const systems::Context<T>& context, MatrixX<T>* P_quasidynamic) const {
  DRAKE_DEMAND(impl_ != nullptr);

  const int num_actuated_dofs = impl_->num_actuated_dofs();
  const int num_unactuated_dofs = impl_->num_unactuated_dofs();
  const Eigen::SparseMatrix<T>& N = EvalQDotToVelocityMap(context);
  const Eigen::SparseMatrix<T> N_unactuated =
      N.bottomRightCorner(num_unactuated_dofs, num_unactuated_dofs);
  ...
  P_quasidynamic->bottomRightCorner(num_unactuated_dofs, num_unactuated_dofs) =
      N_unactuated.transpose() * M_unactuated * N_unactuated / (dt_ * dt_);
  P_quasidynamic->diagonal().head(num_actuated_dofs) =
      impl_->GetActuationStiffnessVector();
}

template <typename T>
const MatrixX<T>& QuasidynamicMultibodyPlant<T>::EvalQuasidynamicPMatrix(
    const systems::Context<T>& context) const {
...
template <typename T>
void QuasidynamicMultibodyPlant<T>::CalcQuasidynamicBVector(
  ...
  const Scratch<T>& scratch = EvalScratch(*this, context, scratch_cache_index_);
  *b_quasidynamic =
      -N.transpose() * impl_->CalcGravityGeneralizedForces(scratch);
  b_quasidynamic->head(num_actuated_dofs) -= k.cwiseProduct(u);
  b_quasidynamic->tail(num_unactuated_dofs) -=
      N_unactuated.transpose() * M_unactuated * N_unactuated *
      x.tail(num_unactuated_dofs) / (dt_ * dt_);
}

template <typename T>
const VectorX<T>& QuasidynamicMultibodyPlant<T>::EvalQuasidynamicBVector(
    const systems::Context<T>& context) const {
...
template <typename T>
void QuasidynamicMultibodyPlant<T>::CalcContact(
    const systems::Context<T>& context,
    std::tuple<MatrixX<T>, VectorX<T>>* contact) const {
  DRAKE_DEMAND(impl_ != nullptr);

  MatrixX<T>& J = std::get<0>(*contact);
  VectorX<T>& phi = std::get<1>(*contact);
  const Eigen::SparseMatrix<T>& N = EvalQDotToVelocityMap(context);

  const Scratch<T>& scratch = EvalScratch(*this, context, scratch_cache_index_);
  impl_->CalcContact(scratch, &J, &phi, get_geometry_smoothing(context));
  J *= N;
}

template <typename T>
const MatrixX<T>& QuasidynamicMultibodyPlant<T>::EvalContactJacobian(
    const systems::Context<T>& context) const {
  return std::get<0>(
...
template <typename T>
const VectorX<T>& QuasidynamicMultibodyPlant<T>::EvalContactSignedDistance(
    const systems::Context<T>& context) const {
  return std::get<1>(
      this->get_cache_entry(contact_cache_index_)
          .template Eval<std::tuple<MatrixX<T>, VectorX<T>>>(context));
...
QuasidynamicMultibodyPlant<T>::EvalContactFrictionCoefficient(
...
template <typename T>
const math::RigidTransform<T>& QuasidynamicMultibodyPlant<T>::EvalPoseInWorld(
    const systems::Context<T>& context, BodyIndex body) const {
  DRAKE_DEMAND(impl_ != nullptr);
  const Scratch<T>& scratch = EvalScratch(*this, context, scratch_cache_index_);
  return impl_->EvalPoseInWorld(scratch, body);
}

template <typename T>
void QuasidynamicMultibodyPlant<T>::CalcCollision(
    const systems::Context<T>& context,
...
template <typename T>
const MatrixX<T>& QuasidynamicMultibodyPlant<T>::EvalCollisionJacobian(
    const systems::Context<T>& context) const {
  return std::get<0>(
...
template <typename T>
const VectorX<T>& QuasidynamicMultibodyPlant<T>::EvalCollisionSignedDistance(
    const systems::Context<T>& context) const {
  return std::get<1>(
      this->get_cache_entry(collision_cache_index_)
          .template Eval<std::tuple<MatrixX<T>, VectorX<T>>>(context));
...
template <typename T>
void QuasidynamicMultibodyPlant<T>::SetMeshcat(geometry::Meshcat* meshcat,
                                               std::string_view name_prefix) {
  DRAKE_DEMAND(impl_ != nullptr);
  impl_->SetMeshcat(meshcat, name_prefix);
}

template <typename T>
systems::EventStatus QuasidynamicMultibodyPlant<T>::DrawMeshcat(
    const systems::Context<T>& context) const {
  DRAKE_DEMAND(impl_ != nullptr);
  const Scratch<T>& scratch = EvalScratch(*this, context, scratch_cache_index_);
...
struct QuasidynamicMultibodyPlantSparseMatrix {
  QuasidynamicMultibodyPlantSparseMatrix(int rows, int cols)
      : matrix(rows, cols) {}
